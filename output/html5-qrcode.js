...
class Html5Qrcode {
  ...
  static SCAN_DEFAULT_IDLE_TIMOUT = 2000;
   ...
   *      - qrboxWidth: width of QR scanning box, this should be smaller than
  ...
  start(
    ...
    config.idleTimout = config.idleTimout
      ? config.idleTimout
      : Html5Qrcode.SCAN_DEFAULT_IDLE_TIMOUT;
    ...
    // qr shaded box
    const isShadedBoxEnabled = config.qrboxWidth != undefined;
    ...
    if (isShadedBoxEnabled) {
      const qrboxSize = config.qrboxWidth;
      if (qrboxSize < Html5Qrcode.MIN_QR_BOX_SIZE) {
        throw (
          "minimum size of 'config.qrboxWidth' is" +
    ...
    const setupUi = (width, height) => {
      const qrboxWidth = config.qrboxWidth;
      const qrboxHeight = config.qrboxHeight ? config.qrboxHeight : qrboxWidth;
      ...
      const shouldShadingBeApplied = isShadedBoxEnabled && qrboxWidth <= height;
      ...
      const qrRegion = shouldShadingBeApplied
        ? this._getShadedRegionBounds(width, height, qrboxWidth, qrboxHeight)
        : defaultQrRegion;
      ...
      if (shouldShadingBeApplied) {
        this._possiblyInsertShadingElement(
          element,
          width,
          height,
          qrboxWidth,
    ...
    const scanContext = () => {
      try {
        ...
        this._idleTimout = config.idleTimout;
        return true;
      } catch (exception) {
    ...
    const foreverScan = () => {
      ...
      $this._foreverScanTimeout = setTimeout(
        foreverScan,
        Math.max(Html5Qrcode._getTimeoutFps(config.fps), this._idleTimout)
      );
      this._idleTimout = 0;
    };
  ...
  _getShadedRegionBounds(width, height, qrboxWidth, qrboxHeight) {
    if (qrboxWidth > width || qrboxHeight > height) {
      throw (
        "'config.qrbox' should not be greater than the " +
    ...
    return {
      x: (width - qrboxWidth) / 2,
      y: (height - qrboxHeight) / 2,
      width: qrboxWidth,
      height: qrboxHeight,
    };
  }

  _possiblyInsertShadingElement(
    element,
    width,
    height,
    qrboxWidth,
    ...
    shadingElement.style.borderTop = `${
      (height - qrboxHeight) / 2
    }px solid #0000007a`;
    shadingElement.style.borderBottom = `${
      (height - qrboxHeight - 10) / 2 // Don't understand why -10, but it's working
    }px solid #0000007a`;
