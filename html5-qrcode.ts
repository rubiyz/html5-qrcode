...
export interface Html5QrcodeCameraScanConfig {
    ...
    qrboxWidth?: number | undefined;
    
    qrboxHeight?: number | undefined;
    ...
    /**
     * Optional, insert a delay between two scans to allow a user 
     * scanning a serie of barcodes to position the phone to the next wanted barecode
     * without getting unwanted scan inbetween. (ie. scanning inside a catalogue with)
     * multiple barcodes close from each others.
     */
...
class InternalHtml5QrcodeConfig implements InternalHtml5QrcodeConfig {
    ...
    public qrboxWidth: number | undefined;
    public qrboxHeight: number | undefined;
    ...
    private constructor(
        ...
        this.qrboxWidth = config.qrboxWidth;
        this.qrboxHeight = config.qrboxHeight;
    }
    ...
    public isShadedBoxEnabled(): boolean {
        return !isNullOrUndefined(this.qrboxWidth);
    }
...
export class Html5Qrcode {
    ...
    public start(
        ...
        if (isShadedBoxEnabled) {
            const qrboxSize = internalConfig.qrboxWidth!;
            if (qrboxSize < Constants.MIN_QR_BOX_SIZE) {
                throw "minimum size of 'config.qrboxWidth' is"
    ...
    */
    private setupUi(
        ...
        const qrboxWidth = internalConfig.qrboxWidth;
        const qrboxHeight = internalConfig.qrboxHeight ? internalConfig.qrboxHeight : qrboxWidth;

        if (qrboxHeight > height) {
        ...
        const shouldShadingBeApplied
            = internalConfig.isShadedBoxEnabled() && qrboxWidth <= height;
        ...
        const qrRegion = shouldShadingBeApplied
            ? this.getShadedRegionBounds(width, height, qrboxWidth)
            : defaultQrRegion;
        ...
        if (shouldShadingBeApplied) {
            this.possiblyInsertShadingElement(
                this.element!, width, height, qrboxWidth, qrboxHeight);
        }
    ...
    private foreverScan(
        ...
        const triggerNextScan = () => {
            ...
            }, Math.max(this.getTimeoutFps(internalConfig.fps), this.idleTimeout));
        };
        this.idleTimeout = 0;
        ...
        this.scanContext(qrCodeSuccessCallback, qrCodeErrorCallback)
            .then((isSuccessfull) => {
                this.idleTimeout = internalConfig.idleTimeout;
    ...
    private possiblyInsertShadingElement(
        ...
        if ((width - qrboxWidth) < 11 || (height - qrboxHeight) < 11) {
          ...
          this.insertShaderBorders(
              ...
              smallSize,
              largeSize + smallSize,
              qrboxHeight + smallSize - largeSize,
              -smallSize,
              true);
          this.insertShaderBorders(
              shadingElement,
